...
class _BaseCapture:
    def __init__(self, out_queue: queue.Queue, block_seconds: float,
                 log_prefix: str):
    ...
    def start(self):
        ...
        print(f"{self.log_prefix} キャプチャ開始: {device['name']} "
...
class LoopbackCapture(_BaseCapture):
    def __init__(self, out_queue: queue.Queue, block_seconds: float = 0.1):
        super().__init__(out_queue, block_seconds, "[audio]")
...
class MicrophoneCapture(_BaseCapture):
    def __init__(self, out_queue: queue.Queue, block_seconds: float = 0.1,
                 device_name: str = ""):
        super().__init__(out_queue, block_seconds, "[mic]")
